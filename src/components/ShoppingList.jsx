...
function ShoppingList({ items = [], onInc, onDec, onDelete, onToggleBought }) {
  ...
  return (
    <div className="rounded-xl border border-blue-100 bg-white p-2 sm:p-3 dark:bg-slate-800 dark:border-slate-700">
      <h2 className="mb-2 text-base sm:text-lg font-medium text-blue-800 dark:text-blue-300">
        Shopping list
      </h2>
      {items.length === 0 ? (
        ...
        <div className="space-y-4">
          {groups.map(([cat, list]) => (
            <section key={cat} className="">
              <h3 className="mb-1 flex items-center gap-2 text-sm font-semibold text-blue-900 dark:text-slate-100">
